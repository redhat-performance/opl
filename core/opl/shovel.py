...
class pluginOpenSearch:
    ...
    def upload(self,args):
        ...
        if args.data_file == None:
            ...
            json_data=json.dumps({"query":{"match":{"endTimestamp":args.end_timestamp}}})
            headers={
                "Content-Type": "application/json"
            }
            jsonFile = open(args.data_file, 'r')
            values = json.load(jsonFile)
            current_doc_in_es=requests.get(f'{args.es_host_url}/{args.es_index}/_search',headers=headers,data=json_data)
    ...
    def args(parser, group_actions):
        ...
        group.add_argument(
            "--data-file",
            help="json file to upload to elastic search"
        )
        group.add_argument(
            "--end-timestamp",
            help="timestamp when the test ended"
        )
...
class pluginHorreum:
    ...
    def upload(self,args):
        if args.horreum_data_file == None:
            raise Exception("Horreum data file is required to work with --horreum-upload")
        elif args.horreum_host == None:
            raise Exception("Horreum host is required to work with --horreum-upload")
        elif args.token == None:
            raise Exception("Authorisation Token is required to work with --horreum-upload")
            ...
            if not test_start or not test_end or test_start == "None" or test_end == "None":
                raise Exception("ERROR: We need start and end time in the JSON we are supposed to upload")
            response = requests.get(f'{args.horreum_host}/api/test/byName/{args.test_name_horreum}',headers=headers)  
            test_id = json.load(response.text)["id"]
            page=0
            while True:
                data={"from":page,"size":100,"sort": [{"date": {"order": "desc"}}]}
                page_data=requests.get(f'{args.horreum_host}/api/run/list/{test_id}',headers=headers,data=data)
                page=page+100
                try:
                ...
                if page_data_len==0:
                    print("INFO: No more Horreum test results to review, proceeding with upload")
                    break
                for runs in page_data_values["runs"]:
                    test_run_id=runs["id"]
                    test_run_data=requests.get(f'{args.horreum_host}/api/run/{test_run_id}')
                    try:
                        json.loads(test_run_data)
                    except ValueError as e:
                        raise Exception(e)
                    test_run_data_values=json.loads(test_run_data)
                    test_run_matcher=test_run_data_values["data"][args.test_job_matcher]
                    if test_run_matcher=="null":
                        print(f'WARNING: Test run {test_run_id} for test {args.test_name_horreum}/{test_id} does not have {args.test_job_matcher}, skipping')
                        continue
                    if test_matcher==test_run_matcher:
                        raise Exception("INFO: Test result found in Horreum, skipping upload")
    ...
    def result(self,args):
        ...
        if args.id_array == None:
            raise Exception("Id array json file is needed to work with --horreum-result")
        elif args.horreum_data_file == None:
            raise Exception("Horreum data file is required to work with --horreum-result")
        else:
            ...
            for i in values:
                ...
                result = subprocess.check_output(curl_command, shell=True).decode('utf-8').strip()
    ...
    def args(parser, group_actions):
        ...
        group.add_argument(
            "--horreum-data-file",
            help="Data file to upload to Horreum"
        )
        group.add_argument(
            "--horreum-host",
            help="Horreum host url"
        )
        group.add_argument(
            "--token",
            help="Authorisation token"
        )
        group.add_argument(
            "--job-name",
            help="Job name"
        ...
        group.add_argument(
            "--id-array",
            help="Variable id array we wish to get timeseries data for"
        )
...
class pluginResultsDashboard:
    ...
    def upload(self,args):
        ...
        if args.status_data is None:
            raise Exception("Status data file is mandatory to work with --results-dashboard-upload")
        elif args.es_host_url is None:
            raise Exception("ES host url is required to work with --results-dashboard-upload")
        elif args.group_name is None:
            raise Exception("Group Name is mandatory to work with --results-dashboard-upload")
        elif args.product_name is None:
            raise Exception("Product Name is mandatory to work with --results-dashboard-upload")
        elif args.test_name is None:
            raise Exception("Test Name is mandatory to work with --results-dashboard-upload")
        else:
            ...
            json_data=json.dumps({"query":{"bool":{"filter":[{"term":{"result_id.keyword":f"{result_id}"}}]}}})
            headers={
                "Content-Type": "application/json"
            }
            current_doc_in_es=requests.get(f'{args.es_host_url}/{args.dashboard_es_index}/_search',headers=headers,data=json_data)
            if json.loads(current_doc_in_es.text)["hits"]["total"]["value"] == 0:
                print("Uploading to results dashboard")
                upload_data=json.dumps({"date":date,"group":args.group_name,"link":link,"product":args.product_name,"release":args.release,"result":result,"result_id":result_id,"test":args.test_name,"version":args.version})
                requests.post(f'{args.es_host_url}/{args.dashboard_es_index}/_doc',headers=headers,data=upload_data)
            else:
    ...
    def args(parser, group_actions):
        ...
        group.add_argument(
            "--group-name",
            help="Name of the group where the product belongs"
        )
        group.add_argument(
            "--product-name",
