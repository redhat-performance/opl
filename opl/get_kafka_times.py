...
class GetKafkaTimes:
    ...
    def store_item(self, item):
        ...
        if (
            len(self.waiting_items) >= self.batches_size or len(self.waiting_items) >= self.remaining_count
        ):
            self.store_now()
                    ...
                    for message in messages:
                        ...
                        if self.custom_methods["message_validation"](value):
                            if self.show_processed_messages:
                                print(
                                    f"Processing {message.timestamp} {topic.topic} {topic.partition} {message.offset} {str(value)[:100]}..."
                                )
                            ...
                            if self.show_dropped_messages:
                                print(
                                    f"Dropping {message.timestamp} {topic.topic} {topic.partition} {message.offset} {str(value)[:100]}..."
                                )
...
def get_kafka_times(custom_methods):
    ...
    with opl.skelet.test_setup(parser) as (args, status_data):
        args.max_quiet_period = datetime.timedelta(seconds=args.max_quiet_period)

        get_kafka_times_object = GetKafkaTimes(args, status_data, custom_methods)
