...
import opl.edge_utils  # noqa: E402
...
def create_principal(cursor):
    ...
    cursor.execute(
        "INSERT INTO management_principal (uuid, username) VALUES (%s, %s) RETURNING id",
        (user_uuid, user_name),
    )
...
def add_principal_to_group(cursor, user_id, group_uuid):
    ...
    cursor.execute(
        "INSERT INTO management_group_principals (group_id, principal_id) VALUES (%s, %s)",
        (group_id, user_id),
    )


def doit(edge_test_data, args, status_data):
    url_base = f"{args.edge_host}{args.edge_url_suffix}"
    ...
    for _ in range(args.tenants_number):
        ...
        edge_test_data.add_account(account, [], APPLICATIONS)
        ...
        logging.info(f"Connecting to DB: {edge_db_conf}")
        connection = psycopg2.connect(**edge_db_conf)
        ...
        for _ in range(args.principals_number):
            ...
            edge_test_data.add_account(account, [user_name], [])
    ...
    return edge_test_data


def main():
    parser = argparse.ArgumentParser(
        description="Create bunch of EDGE tenants and populate them",
    ...
    parser.add_argument(
        "--groups-number", type=int, default=0, help="Number of groups per tenants to create"
    )

    parser.add_argument(
        "--roles-number", type=int, default=0, help="Number of roles per group to create"
    )
    ...
    parser.add_argument(
        "--test-data-file",
        default="/tmp/edge-test-data.json",
        help="File where to add test data created here",
    )

    parser.add_argument(
        "--edge-url-suffix",
        dest="edge_url_suffix",
        default=os.getenv("EDGE_URL_SUFFIX", "/api/edge/v1"),
        help="EDGE host URL suffix (also use env variable EDGE_URL_SUFFIX)",
    )

    parser.add_argument(
        "--edge-host",
        dest="edge_host",
        default=os.getenv("EDGE_HOST", "http://edge.qa.svc:8080"),
        help="EDGE host to test (also use env variable EDGE_HOST)",
    )
    opl.args.add_edge_db_opts(parser)
    with opl.skelet.test_setup(parser) as (args, status_data):
        edge_test_data = opl.edge_utils.EdgeTestData(args.test_data_file)
        return doit(edge_test_data, args, status_data)
